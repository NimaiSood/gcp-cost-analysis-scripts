#!/usr/bin/env python3
"""
Script to list the top 50 projects linked to a specific billing account.
...
def main():
    ...
    try:
        ...
        # Limit to first MAX_PROJECTS for processing
        projects_to_process = projects[:MAX_PROJECTS]
        logger.info(f"\n💰 Analyzing costs for {len(projects_to_process)} projects (limited from {len(projects)} total)...")
        ...
        for i, project in enumerate(projects_to_process, 1):
            ...
            logger.info(f"📊 [{i}/{len(projects_to_process)}] Analyzing project: {project_id}")
            ...
            # Progress indicator
            if i % 10 == 0:
                logger.info(f"✅ Processed {i}/{len(projects_to_process)} projects")
        ...
        # Since we already limited to MAX_PROJECTS, use all enhanced_projects
        top_projects = enhanced_projects
