...
MAX_WORKERS = 5  # Reduced workers to avoid rate limits
...
def process_project(project_id: str) -> Dict[str, Any]:
    """Process a single project to find unused resources (only disks and IPs for now)."""
    ...
    try:
        ...
        # Skip snapshots and buckets for now to avoid timeout issues
        # These can be added back later once the basic functionality is working
        
        # If we had any errors but still got some results, mark as partial success
        if result['error'] and (result['unattached_disks'] or result['unused_ips']):
...
def generate_excel_report(all_results: List[Dict[str, Any]], billing_account_id: str) -> str:
    """Generate an Excel report with unused resources."""
    ...
    for result in all_results:
        ...
        if has_unused_resources(result):
            summary_data.append({
                ...
                'Total Unused Resources': (len(result['unattached_disks']) + len(result['unused_ips'])),
...
def main():
    ...
    print(f"  • Focus: Unattached Disks and Unused IPs")
    ...
    if all_results_with_resources:
        ...
        print("-" * 70)
        print(f"{'Project ID':<35} {'Disks':<6} {'IPs':<4} {'Status'}")
        print("-" * 70)
        
        # Sort by total unused resources (descending)
        sorted_results = sorted(all_results_with_resources, 
                               key=lambda x: (len(x['unattached_disks']) + len(x['unused_ips'])), 
                               reverse=True)
        
        for result in sorted_results:
            status_icon = "✅" if result['status'] == 'success' else "⚠️" if result['status'] == 'partial' else "❌"
            print(f"{result['project_id']:<35} "
